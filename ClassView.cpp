...
void CClassView::FillClassView(CTransitionCollection* pTransitionList)
{
	m_wndClassView.DeleteAllItems();

	hRootStates = m_wndClassView.InsertItem(_T("States"), 0, 0);
	m_wndClassView.SetItemState(hRootStates, TVIS_BOLD, TVIS_BOLD);
//	m_wndClassView.Expand(hRootStates, TVE_EXPAND);


	hRootTransitions = m_wndClassView.InsertItem(_T("Transitions"), 0, 0);
	m_wndClassView.SetItemState(hRootTransitions, TVIS_BOLD, TVIS_BOLD);

	HTREEITEM hResults = m_wndClassView.InsertItem(_T("Results"), 0, 0);
	m_wndClassView.SetItemState(hResults, TVIS_BOLD, TVIS_BOLD);

	HTREEITEM hStateProbabilities = m_wndClassView.InsertItem(_T("State Probabilities"), 0, 0, hResults);
	m_wndClassView.Expand(hResults, TVE_EXPAND);

	if(pTransitionList == NULL)
		return;

	CStateCollection* pStateList = pTransitionList->GetStateList();

	if(pStateList == NULL)
		return;


	std::vector<std::string> states = pStateList->GetStateNames();
	CString temp;
	HTREEITEM hItem;
	...
	for(int i=0; i<states.size() ; i++)
	{
		...
		pInfo = (DWORD_PTR)pStateList->GetState(i);

		hItem = m_wndClassView.InsertItem(temp, 3,3, hRootStates);
		m_wndClassView.SetItemData(hItem, pInfo);

		hItem = m_wndClassView.InsertItem(temp, 3,3, hStateProbabilities);
		m_wndClassView.SetItemData(hItem, pInfo);
	}

	m_wndClassView.Expand(hRootStates, TVE_EXPAND);
	m_wndClassView.Expand(hStateProbabilities, TVE_EXPAND);

	if( pTransitionList->GetTransitionCount() == 0 )
		return;
	...
	HTREEITEM hTransition;
	HTREEITEM hDistribution;

	for(int i=0; i< pTransitionList->GetTransitionCount(); i++)
	{
		pTransition = pTransitionList->GetTransition(i);
		
		sTransition.Format(L"%s -> %s", 
			pTransition->GetFromState()->GetStateName(),
			pTransition->GetToState()->GetStateName() );

		hTransition = m_wndClassView.InsertItem(sTransition, 3,3, hRootTransitions);
		m_wndClassView.SetItemData(hTransition, (DWORD_PTR)pTransition);

		for(int j=0; j<pTransition->GetDistributionCount(); j++)
		{
			pDistribution = pTransition->GetDistribution(j);
			temp = CString( sDistributions[ pDistribution->GetDistributionType() ] );
			hDistribution = m_wndClassView.InsertItem(temp, 3,3, hTransition);
			m_wndClassView.SetItemData(hDistribution, (DWORD_PTR)pDistribution);
		}

		m_wndClassView.Expand(hTransition, TVE_EXPAND);
	}


	m_wndClassView.Expand(hRootTransitions, TVE_EXPAND);
