...
BOOL CRedirector::Printf(LPCTSTR pszFormat, ...)
{
	...
	DWORD dwWritten; //(LPCTSTR)strInput
	return ::WriteFile(m_hStdinWrite, L"DIR\r\n",
...
{
	//TRACE("%s", pszOutput);
	m_pOutput->AppendText(CString(pszOutput));
}

void CRedirector::WriteStdError(LPCSTR pszError)
{
	//TRACE("%s", pszError);
	m_pOutput->AppendText(CString(pszError));
}
...
CString CRedirector::ErrorString()
    {
	...
	{ /* failed */
	 ...
	 CString fmt;
	 CString t;
	 fmt = L"Unknown error code %08x (%d)";
